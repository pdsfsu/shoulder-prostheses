...
def circle_mask(img, min_dist=20, param_1=100, param_2=30, min_radius=0, max_radius=0):
    ...
    param_2 = 30
    dp = 1
    circles = cv2.HoughCircles(temp_img, cv2.cv.CV_HOUGH_GRADIENT, 1, min_dist, param1=100, param2=param_2, minRadius=min_radius,maxRadius=max_radius)
...
def unsmoothed(img, eqImg, seed, th, img_file, out_info, man_img):
    ...
    # t = evaluation.findTotals(out_img_h, man_img)
    # f = open('h_all.txt', 'a')
    # f.write(img_file + " " + str(t[0]) + " " + str(t[1]) + " " + str(t[2]) + " " + str(t[3]) + "\n")
    # f.close()
...
def BilateralFilters(img, eqImg, seed, th, img_file, out_info, man_img):
    img_b = cv2.bilateralFilter(img, 5, 100, 100)
    img_bh = cv2.bilateralFilter(eqImg, 5, 50, 50)
...
def singlethresholdOtsu(img, mask=None):
...
for line in nfile:
    ...
    # averageFilters(combined_img, h, seed, th, img_file, out_info, masked_man)
    # GaussianFilters(combined_img, h, seed, th, img_file, out_info, masked_man)
    BilateralFilters(combined_img, h, seed, th, img_file, out_info, masked_man)
    otsuOne(img, img_file, masked_man, c_mask)
    # otsuTwo(img, img_file, masked_man, c_mask)
    otsuThree(img, img_file, masked_man, c_mask)
